...
resource "aws_security_group" "ecs_tasks" {
  ...
  description = "Allow traffic to ECS tasks from ALB and internal services"
  vpc_id      = aws_vpc.main.id
  ...
  # Allow internal communication for Prometheus scraping (ECS Exporter)
  ingress {
    from_port   = 9779
    to_port     = 9779
    protocol    = "tcp"
...
# # Then in your container definition, use:
# container_definitions = jsonencode([
#   {
#     name  = "koronet-web-server"
  ...
  container_definitions = jsonencode([
    {
      ...
      environment = [
        ...
        {
          name  = "DB_HOST"
          value = "postgres.example.com"  # Update with actual DB host
        },
        {
          name  = "REDIS_HOST"
          value = "redis.example.com"     # Update with actual Redis host
        }
      ]
      healthCheck = {
